...
for subID in $subID; do
	...
	cond=$(</projects/kg98/Thapa/DiCER/1_ParametersProject/3_DiCERParamsTests/3_IdentificationScoreTest/ScriptsUsed/Conditions.txt)
		...
		fmriprep_dir=/home/ttha0011/kg98/Thapa/DiCER/1_ParametersProject/3_DiCERParamsTests/3_IdentificationScoreTest/2_UCLA_dataset
		Anat_dir=$fmriprep_dir/$subID/'anat'
		DBScan_dir=$fmriprep_dir/$subID/'dbscan'
		...
		Schaefer_Atlas=/home/ttha0011/kg98/Thapa/DiCER/1_ParametersProject/3_DiCERParamsTests/3_IdentificationScoreTest/ScriptsUsed/Schaefer2018_300Parcels_7Networks_order_FSLMNI152_3mm.nii.gz
		...
		echo -e "\nFlipping T1_to_MNI warp coefficients to MNI_to_T1 warp. You can use this to convert the Schaefer's atlas to T1 space, as it is currently in MNI space\n"
		...
		--out=$Out_dir/$subID'_task-rest_variant-'$cond'_Schaefer_space-T1' \
		--interp=nn;

		#Step 10: Get matrix to convert T1 to func space.
		echo -e "\nGetting matrix to convert T1 to functional image space\n"
		flirt -ref $fmriprep_dir/$subID/dbscan/$subID'_task-rest_bold_space-MNI152NLin2009cAsym_variant-'$cond'.nii.gz' \
		-in $Anat_dir/$subID'_T1w_space-MNI152NLin2009cAsym_preproc.nii.gz' \
		-dof 6 \
		...
		flirt -ref $fmriprep_dir/$subID/dbscan/$subID'_task-rest_bold_space-MNI152NLin2009cAsym_variant-'$cond'.nii.gz' \
		-in $Out_dir/$subID'_task-rest_variant-'$cond'_Schaefer_space-T1' \
		-init $Out_dir/$subID'_task-rest_variant-'$cond'_T1_to_func.mat' \
		-applyxfm \
		-interp nearestneighbour \
		-out $Out_dir/$subID'_task-rest_variant-'$cond'_Schaefer_space-func.nii.gz';

		#Step 12: Use fslmeants, and the PFC sphere_space-func mask to extract the timeseries from the preproc_bold_smooth.nii file.
		fslmeants -i $fmriprep_dir/$subID/dbscan/$subID'_task-rest_bold_space-MNI152NLin2009cAsym_variant-'$cond'.nii.gz' \
		-o $Out_dir/$subID'_task-rest_variant-'$cond'_Schaefer_ts.txt' \
		--label=$Out_dir/$subID'_task-rest_variant-'$cond'_Schaefer_space-func.nii.gz' \
		--transpose; 
