...
%Step 8: Flatten distance matrix
...
for i = 1:numSubs
    ...
    % Get FD
    metadata.fdJenk{i} = GetFDJenk(mov);
    ...
    metadata.FDfmriprep(i) = mean(str2num(conf.FramewiseDisplacement(2:end,:)));
    
    %Calculate GSSD
    metadata.GSSD(i) = std(conf.GlobalSignal);
    % ------------------------------------------------------------------------------
