...
%p = genpath('E:\toolbox\MemToolbox-1.1.0');
addpath(p);
...
% Different loads
loadNum = [6]; %[2;4;6]; %capacity is typically around 3. To super capacitate average load 4&6.
...
for idx = 1:length(ID)
    ...
    for loadx = 1:length(loadNum)
        ...
        errorAll.(['load',num2str(loadNum(loadx))]){idx} = [dataOut.ses1.(['err',num2str(loadNum(loadx))]),...
         ...
         posteriorSamples = MemFit(DataIn, StandardMixtureModel, 'Verbosity',2);
...
Table.Properties.VariableNames = {'ID', 'GuessRate', 'Precision', 'wmCapacity', 'meanPrecision4_6', 'meanwmCapacity4_6'};
% writetable(Table, [pathOut,'ColourWheelTaskAll.csv']);

% Save the data
% save([pathOut,'ColourWheelTaskAll.mat'],'guessRate', 'wmCapacity', 'precision');
